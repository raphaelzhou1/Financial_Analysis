 ...
 */
/*
 Visitor id removed as requested on 27 sept 2017
 /*
 ============== END VisitorAPI Base Code ============
 */
/*
 if (typeof(visitor) == 'undefined'){
 var visitor = Visitor.getInstance("7C3F11DA57FCC1D50A495C21@AdobeOrg", {
 trackingServer: "citiresearch.sc.omtrdc.net",
 //trackingServerSecure: "citiresearch.sc.omtrdc.net",
 marketingCloudServer: "citiresearch.sc.omtrdc.net",
 //marketingCloudServerSecure: "citiresearch.sc.omtrdc.net",
 loadTimeout: 30000
 });
 }
 */
...
if (document.location.href.search("www.citivelocity.com") > -1) {
	s.account="citirprod1";
}
else {
	s.account="citiresearchuat1";
}
//s.account="citiresearchuat1"
/* Conversion Config */
...
/*
 ...
 s.visitor = Visitor.getInstance("135C210654F72CED0A4C98A7@AdobeOrg");

 s.eVar17 = s.visitor.getMarketingCloudVisitorID();

 }else{

 s.eVar17 = "MCMID not available";
...
/*s.linkInternalFilters="javascript:,.,citivelocity.com" */
...
/*
 s.visitorNamespace="citiresearch"
 */
s.trackingServer="citiresearch.sc.omtrdc.net"
...
//Custom Page Load
    ...
    s.eVar40 =s.getLoadTimeDim();
    if(s.eVar40)
        var load_time = s.eVar40;
    s.events=s.apl(s.events,'event3='+load_time,',',1);
...
//Capture URL
    s.prop3=s.eVar12=window.location.href;

    /* For Previous Page value */
    s.prop10=s.eVar10=s.getPreviousValue(s.pageName,'gpv','');
...
//MID
    ...
    /*
     ...
     s.prop12=s.eVar17=s.marketingCloudVisitorID;
     else
     s.prop12=s.eVar17="VisitorAPI Missing";

     */


    s.eVar69 = s.trackLoadTimings();
...
//Copy Content Type
    ...
    if(s.inList("event11", s.events))
    {
        // Time between events
        s.eVar49 = s.getTimeBetweenEvents('event11','f','event11','f','s_tbe_ev49',.02083);
    }
...
s.getLoadTimeDim=new Function("",""+"var t='';if(typeof performance!=='undefined'){t=(new Date()).getTim"+"e()-performance.timing.requestStart;t=t/1000;t=(t>=0&&t<500)?t.toFi"+"xed(1):'';}return t;");
...
s.trackLoadTimings= function() {
    // "browserPageLoadTime" : (s.customFrontPageLoadTime+s.serverRequestProcessTime).toString(),
    //     "serverRequestProcessTime" : (s.serverRequestProcessTime).toString(),
        ...
        if (window.performance && window.performance.timing) {
            ...
            for(var i = 0, l = eventNames.length; i < l; i++) {
                ...
                if (evt && evt > 0) {
                    ...
                    events[eventNames[i]] = eventTime.toString();
            ...
            //	 s.log('timing', 'timingType calculated: totalPageLoadTime:'+timingProfile['totalPageLoadTime']);
            return (s.customFrontPageLoadTime + s.timestampAtIndexHead - timingData.navigationStart).toString();
            //   $.extend(timingProfile, events);

        } else {

//		 s.log('timing', 'timingType calculated: browserPageLoadTime:'+timingProfile['browserPageLoadTime']);
            return (s.customFrontPageLoadTime+s.serverRequestProcessTime).toString();

        }
        /*           var piwikEvent= {trackingType:"customTrackingCall",
         trackingData:
         new rpu.PiwikCustomTrackingData(rpu.PiwikTrackingEventTypeEnum.NON_PAGE,
         "trackLoadTimings", timingProfile
         )};
         rpu.publishPiwikEvent(piwikEvent);
         */
    } catch (exception){
        //     s.log('timing', 'failed to get window.performance');
    }
};
